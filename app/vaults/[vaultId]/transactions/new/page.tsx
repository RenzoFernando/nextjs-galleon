...
function validateTransactionForm(form: TransactionFormState): string | null {
  ...
  if (
    ...
    (!Number.isInteger(Number(form.linkedTransactionId)) || Number(form.linkedTransactionId) < 1)
  ) {
    ...
    <AppShell>
      <section className="mx-auto grid w-full max-w-6xl gap-6 lg:grid-cols-[0.85fr_1.15fr]">
        <aside className="rounded-3xl border border-[#B39F84]/30 bg-[#19242E] p-8 shadow-2xl shadow-black/40">
          <Link
            href={`/vaults/${vaultId}/transactions`}
          ...
          <p className="mt-8 text-sm uppercase tracking-[0.35em] text-[#B39F84]">
            Nuevo movimiento
          </p>
          ...
          <div className="grid gap-5">
            <div className="grid gap-5 md:grid-cols-2">
              <label className="grid gap-2 text-sm font-semibold text-[#F2E8D5]">
                ...
                <select
                  value={form.type}
                  onChange={(event) => setField("type", event.target.value as TransactionType)}
              ...
              <label className="grid gap-2 text-sm font-semibold text-[#F2E8D5]">
                ...
                <input
                  ...
                  onChange={(event) => setField("amountMinor", event.target.value)}
                  className="rounded-2xl border border-[#B39F84]/25 bg-black/30 px-4 py-3 text-[#F2E8D5] outline-none transition focus:border-[#B39F84]"
                  placeholder="500"
                />
                <span className="text-xs font-normal text-[#D6CCA8]/60">
                  Usa números enteros mayores o iguales a 1.
                </span>
              </label>
            </div>
            ...
            <div className="grid gap-5 md:grid-cols-2">
              <label className="grid gap-2 text-sm font-semibold text-[#F2E8D5]">
                ...
                <select
                  value={form.currency}
                  onChange={(event) => setField("currency", event.target.value as CurrencyCode)}
                  className="rounded-2xl border border-[#B39F84]/25 bg-black/30 px-4 py-3 text-[#F2E8D5] outline-none transition focus:border-[#B39F84]"
                >
            ...
            <label className="grid gap-2 text-sm font-semibold text-[#F2E8D5]">
              ...
              <input
                ...
                onChange={(event) => setField("linkedTransactionId", event.target.value)}
                className="rounded-2xl border border-[#B39F84]/25 bg-black/30 px-4 py-3 text-[#F2E8D5] outline-none transition focus:border-[#B39F84]"
